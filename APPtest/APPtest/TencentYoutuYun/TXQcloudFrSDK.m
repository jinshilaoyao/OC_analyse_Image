...
- (void)sendRequest:(NSMutableDictionary *) postData mothod:(NSString *) mothod successBlock:(HttpRequestSuccessBlock)successBlock failureBlock:(HttpRequestFailBlock)failureBlock {
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue new] completionHandler:^(NSURLResponse *response, NSData *dataResult, NSError *connectionError) {
...
- (void)namecardOcr:(UIImage *)image sessionId:(NSString *)sessionId successBlock:(HttpRequestSuccessBlock)successBlock failureBlock:(HttpRequestFailBlock)failureBlock
{
    ...
    [self sendRequest:json mothod:@"/ocrapi/generalocr" successBlock:successBlock failureBlock:failureBlock];
}
